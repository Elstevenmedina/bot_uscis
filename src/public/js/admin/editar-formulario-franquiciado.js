    ...
    $btnGuardarCambios.onclick = () =>{
        if($Titulo.value == '' || $TiempoEstimado.value == "" || $Precio.value == "" || $Precio.value <=0){
            ...
            fetch ('/api/editar-formulario-general-franquiciado',{
                method: 'POST',
    ...
    const actualizarOrden = (items) =>{
        fetch(`/api/actualizar-paginas-franquiciado/${_idPagina}`,{
            method: 'POST',
    ...
    const crearPagina = (dataEnvio) =>{
        fetch ('/api/crear-pagina-franquiciado',{
            method: 'POST',
            body: JSON.stringify(dataEnvio),
            headers: {
                'Content-Type': 'application/json'
    ...
    const evaluarPrecios = (idPagina, idFormulario) =>{
        ...
        fetch('/api/evaluar-precios-franquiciado',{
            method: 'POST',
                ...
                Swal.fire({
                    ...
                    if(result.isConfirmed){
                       ...
                       Swal.fire({
                        ...
                        fetch(`/api/precios-pagina-inputs-franquiciado`,{
                            method: 'POST',
                            body: JSON.stringify(dataEnvio),
                            headers: {
                                'Content-Type': 'application/json'
    ...
    const evaluarConfiguracion = (idPagina, idFormulario) =>{
        ...
        fetch(`/api/evaluar-configuracion-franquiciado`,{
            method: 'POST',
            body: JSON.stringify({idPagina, idFormulario}),
            headers: {
                'Content-Type': 'application/json'
                ...
                Swal.fire({
                    ...
                    if(result.isConfirmed){
                       ...
                       Swal.fire({
                        ...
                        fetch(`/api/configurar-pagina-inputs-franquiciado`,{
                            method: 'POST',
                            body: JSON.stringify(dataEnvio),
                            headers: {
                                'Content-Type': 'application/json'
    ...
    d.addEventListener('click', e=>{
        ...
        if(e.target.textContent == "Editar"){
            ...
            fetch(`/api/formulario-pagina-franquiciado/${idFormulario}`,{
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
        ...
        if(e.target.textContent == "Eliminar"){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    let idFormulario = e.target.dataset.idformulario;
                    let idPagina = e.target.dataset.idpagina;
                    fetch(`/api/eliminar-pagina-franquiciado/${idFormulario}`,{
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json'
                        },
        ...
        if(e.target == $btnCrearPagina){
            fetch(`/api/solicitar-formulario-franquiciado/${$TituloSeccion.dataset._id}`)
            .then((data) =>{
                return data.json()
            })
            .then((data) =>{
