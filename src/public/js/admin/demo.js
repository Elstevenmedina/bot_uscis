const d = document,
  ...
  $insertarContador = d.getElementById("insertarContador"),
...
const solicitarPagina = (_id, pagina, tipo, campoSeleccionado) => {
  Swal.fire({
...
solicitarPagina(_idFormulario, paginaActual, "siguiente"); //aqui
...
const crearPaginaFinal = () => {
  ...
  animacion = "animate__animated animate__slideInRight";
  styles = `style="width:75vw; margin-top: 80px; margin-left:12%; margin-right: 12%; border-radius: 30px !important; "`;
  if (+vw <= 1024) {
    styles = `style="margin-left:12%; margin-right: 12%; border-radius: 30px !important; "`;
  }
...
const cargarPagina = (pagina, paginasTotales, tipo, numeroPagina) => {
  ...
  for (i = 0; i < pagina.Inputs.length; i++) {
    ...
    if (campo.Tipo == "checkbox") {
      ...
      let precioInputEncontrado = preciosInputs.find((data) => data._idCampo == campo._id);
      ...
      input = `
                    <div class="${col}">
                        <div class="${mb} border shadow p-3 mb-5 bg-white rounded">
      ...
      input = `
                    <div class="${col}">
                        <div class="${mb}">
                            <label  for="${campo._id}">${campo.Nombre}</label>
                            <textarea data-precio="0" 
                            data-accion="Suma"  class="form-control shadow-inputs inputForm" id="${campo._id}" name="${campo._id}" 
                            required="${Opcional}" cols="30" rows="10" placeholder="${campo.Nombre}:">${registro.valor}</textarea>
                        </div>
                    </div>
                `;
    } else {
  ...
  Swal.fire({
    ...
    html: `
              ...
              <p>Aceptamos pagos por la plataforma de transferencia ZELLE. 
               ...
               El email para transacciones ZELLE es <b>elretousallc@gmail.com</b> bajo el nombre EL RETO USA LLC.
               Enviar el capture a nuestro <b>WhatsApp 954-887-8140</b> para finalizar el proceso. 
              ...
              <p><b>Email Zelle </b>: elretousallc@gmail.com</p>
...
d.addEventListener("change", (e) => {
  if (e.target.classList.contains("inputForm")) {
    if (e.target.type != "file") {
      ...
      console.log(e.target.dataset.accion);
